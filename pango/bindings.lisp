...
(cffi:defcfun ("g_free" g_free) :void
  (object :pointer))
...
(defun get-all-fonts (&optional (font-map (pango_cairo_font_map_get_default)))
  (cffi:with-foreign-objects ((families :pointer)
			      (c :int))
    (pango_font_map_list_families font-map families c)
    
    (prog1 (loop for i from 0 to (1- (cffi:mem-ref c :int))
	      collect (let ((family (cffi:mem-aref 
				     (cffi:mem-ref families :pointer)
				     :pointer i)))
			(list
			 
			 (pango_font_family_get_name family)
			 (unless (zerop (pango_font_family_is_monospace family))
			   :monospace)
			     
			 
			 (cffi:with-foreign-objects ((faces :pointer)
						     (fcount :int))
			   (pango_font_family_list_faces family faces fcount)
			   (prog1 (loop for i from 0 to (1- (cffi:mem-ref fcount :int))
			 	     collect (let ((face (cffi:mem-aref
			 				  (cffi:mem-aref faces :pointer)
			 				  :pointer i)))
					       (list (pango_font_face_get_face_name face)
						     (cffi:with-foreign-objects ((sizes :pointer)
										 (scount :int))
						       (pango_font_face_list_sizes face sizes scount)
						       (prog1 (if (zerop (cffi:mem-ref scount :int))
								  :any
								  (loop for i from 0 to (1- (cffi:mem-ref scount :int))
								     collect (let ((ssize (cffi:mem-aref
											   (cffi:mem-aref sizes :pointer)
											   :pointer i)))
									       (cffi:mem-aref ssize :int))))

							 (g_free (cffi:mem-ref sizes :pointer)))))))
			     (g_free (cffi:mem-ref faces :pointer)))))))

      
      (g_free (cffi:mem-ref families :pointer)))))
